...
if($rowar['score']==0){
   ...
   //Add to skill result
   if($q==5){
       $obrec=new Recommendation();
       $rec_type="learningObject";
   ...
   while($rowlo=$resultlo->fetch(PDO::FETCH_ASSOC)){
       ...
       $noclo=$obq->viewResultcLO($attempt_id, $rowlo['lo_id']);
                      ...
                      if($stat==1){
                          ...
                          $obrec->addRecommendation($row['learn_id'], $rec_type, $rowlo['lo_id'], $status, $rec_cat,$statu);
                      }else{
                          $status="Partial";
    ...
    <div class="progress-bar" role="progressbar" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100" style="width:<?php echo $score; ?>%">
      <?php echo round($score,2); ?>
        ...
        <form 
            ...
            <?php if($q==5) { ?>
            action="skillquizresult.php?course_id=<?php echo $course_id; ?>&concept_id=<?php echo $concept_id; ?>&en_id=<?php echo $en_id; ?>&attempt_id=<?php echo $attempt_id; ?>"
            <?php } else { ?>
