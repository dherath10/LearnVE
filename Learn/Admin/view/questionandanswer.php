...
$(document).ready(function() {
  ...
  <body class="hold-transition sidebar-mini">
          ...
          <div class="row">
            <div class="col-xs-12"><!-- /.box -->
                ...
                <div class="box-header">
                    <h3 class="al">Questions &amp; Answers</h3>
                </div><!-- /.box-header -->
	...
	$ob=new Question();
$result=$ob->viewQuestion();

$obc=new Course();
	
		
		while($rowx=$result->fetch(PDO::FETCH_BOTH)){
		$resultc=$obc->viewCOCONLO($rowx['lo_id']);
		$rowc=$resultc->fetch(PDO::FETCH_BOTH);
                $resultaq=$ob->viewAnswerQ($rowx['q_id']);
		?><tr <?php if(isset($_REQUEST['msg']) && $count==1){ ?>
                class="success" 
           <?php } ?>>
                
                
                <td><?php echo $rowx['q_id']; ?></td>
                <td><?php echo $rowx['q_des']; ?></td>
                <td><?php echo $rowx['level_name']; ?></td>
                
                <td><?php echo $rowx['level_weigh']; ?></td>
                <td>
                    <?php
                        $count=0;
                        while($rowaq=$resultaq->fetch(PDO::FETCH_ASSOC)) {
                            $count++;
                           // echo $rowaq['ans_status'];
                            ?>
                        <span <?php  if($rowaq['ans_status']==1) echo "class='alert-success'"; ?>> 
                        <?php 
                            echo "($count)".$rowaq['ans_des']." &nbsp;";
                          echo "</span>"; 
                        }
                        ?>
                  ...
                  <td><?php echo $rowc['course_name']."-".$rowc['concept_name']; ?></td>
                <td>
              
                    <a href="../view/qnacontroller.php?id=<?php echo $rowx['q_id']; ?>&status=Update">
                        <i class="glyphicon glyphicon-pencil"></i></button>
                    </a>
                    <a href="../controller/qnacontroller.php?id=<?php echo $rowx['q_id']; ?>&status=Delete">
